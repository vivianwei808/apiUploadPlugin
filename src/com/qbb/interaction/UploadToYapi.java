...
public class UploadToYapi extends AnAction {
    ...
    private final String helpStr = ",please see http://";
            ...
            if (modules.length > 1) {
                ...
                for (int i = 0; i < moduleList.length; i++) {
                    if (virtualFile.contains(moduleList[i])) {
                        ...
                        yapiUrl = projectConfig.split(moduleList[i] + "\\.pmoApiUrl\">")[1].split("</")[0];
//                        projectType = projectConfig.split(moduleList[i] + "\\.projectType\">")[1].split("</")[0];
                ...
                yapiUrl = projectConfig.split("pmoApiUrl\">")[1].split("</")[0];
            ...
            Notification error = notificationGroup.createNotification("get config error:" + e2.getMessage()+helpStr, NotificationType.ERROR);
            Notifications.Bus.notify(error, project);
            return;
        }
        // 配置校验
        if (Strings.isNullOrEmpty(projectToken) || Strings.isNullOrEmpty(projectId) || Strings.isNullOrEmpty(yapiUrl)) {
            Notification error = notificationGroup.createNotification("please check config,[projectToken,projectId,pmoApiUrl]"+helpStr, NotificationType.ERROR);
            Notifications.Bus.notify(error, project);
            return;
        }
        // 判断项目类型
